...
{
    public sealed class HealthCheckOperations : IDisposable
    {
        #region Constructors
        ...
        public HealthCheckOperations(
            WatchdogService svc, ITelemetryLogger telemetry, TimeSpan interval, CancellationToken token, TimeSpan timeout = default(TimeSpan))
        {
            if (null == svc)
            {
        ...
        /// </summary>
        internal readonly WatchdogService _service = null;
        ...
        /// </summary>
        internal readonly ITelemetryLogger _telemetry = null;
        ...
        /// <summary>
        /// Gets the HealthCheck dictionary. If it doesn't already  exist it is created.
        /// </summary>
        public async Task<IReliableDictionary<string, HealthCheck>> GetHealthCheckDictionaryAsync()
        {
        ...
        internal async Task EnumerateHealthChecksAsync()
        {
            ...
            using (ITransaction eTx = this._service.StateManager.CreateTransaction())
            {
                ...
                Microsoft.ServiceFabric.Data.IAsyncEnumerator<KeyValuePair<long, WatchdogScheduledItem>> ae =
        ...
        internal async Task<HealthCheck> ExecuteHealthCheckAsync(HealthCheck hc, Partition partition)
        {
            ...
            IDictionary<string, string> props = new Dictionary<string, string>(new KeyValuePair<string, string>[]
               {
                    new KeyValuePair<string, string>("Partition", hc.Partition.ToString()),
                    new KeyValuePair<string, string>("ServiceName", hc.ServiceName.AbsoluteUri)
               });
            ...
            try
            {
                ...
                this._telemetry.TrackAvailability(hc.Key, hc.Name, TimeSpan.FromMilliseconds(hc.Duration), props, success);
            ...
            //    this._telemetry.TrackAvailability(hc.Key, hc.Name, TimeSpan.FromMilliseconds(hc.Duration), props, false);
        ...
        public async Task<IList<HealthCheck>> GetHealthChecksAsync(string application = null, string service = null, Guid? partition = null)
        {
            ...
            try
            {
                using (ITransaction tx = this._service.StateManager.CreateTransaction())
                {
                    ...
                    Microsoft.ServiceFabric.Data.IAsyncEnumerable<KeyValuePair<string, HealthCheck>> list =
                        await hcDict.CreateEnumerableAsync(tx, (s) => { return s.StartsWith(filter); }, EnumerationMode.Ordered).ConfigureAwait(false);
                    Microsoft.ServiceFabric.Data.IAsyncEnumerator<KeyValuePair<string, HealthCheck>> asyncEnumerator = list.GetAsyncEnumerator();
