...
namespace WatchdogService
{
    public sealed class CleanupOperations : IDisposable
    {
        ...
        public CleanupOperations(TimeSpan interval, CancellationToken token, TimeSpan timeout = default(TimeSpan))
        {
        ...
        internal async Task CleanupDiagnosticTablesAsync()
        {
            ...
            try
            {
                ...
                foreach (string tableName in this._tablesToInspect)
                {
                    ...
                    if (true == await table.ExistsAsync())
                    {
        ...
        internal async Task<int> EnumerateTableItemsAsync(CloudTable table)
        {
            ...
            {
                // Execute the query.
                TableQuerySegment queryResult = await table.ExecuteQuerySegmentedAsync(query, tct).ConfigureAwait(false);
