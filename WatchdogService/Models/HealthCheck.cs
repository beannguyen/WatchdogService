...
namespace WatchdogService.Models
{
    ...
    public struct HealthCheck : IEquatable<HealthCheck>, ICloneable<HealthCheck>
    {
        ...
        public HealthCheck(
            ...
            string actorId = default(string),
            string actorKind = default(string))
        {
        ...
        public string Key
        {
            get
            {
                if (null == this.ServiceName) return null;

                if (this.ServiceName.IsAbsoluteUri)
                {
                    return $"{this.ServiceName.AbsolutePath}/{this.Name}/{this.Partition}";
                }
                else
                {
                    return null;
